...
#define BUF_SIZE    4   // different echo_epollserv.c
...
int main(int argc, char *argv[]){  
    ...
    while(1){ 
		...
		puts("return epoll_wait");  // different echo_epollserv.c
		for(i=0; i<event_cnt; i++){
			if(ep_events[i].data.fd == serv_sock){
				adr_sz = sizeof(clnt_adr);
				clnt_sock = accept(serv_sock,(struct sockaddr *)&clnt_adr,&adr_sz);  
				//处理请求套接字生成
				event.events = EPOLLIN | EPOLLET;  // 边缘触发 different echo_epollserv.c
